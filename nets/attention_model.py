...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_noop:
            ...
            # node_dim=3 # 用户的最大功率限制,用户的权重,用户当前信道质量
            node_dim = 6 # 用户最大功率限制、用户权重、用户当前信道质量 tips:双天线，实部虚部，共4维
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            ...
            embeddings, _ = self.embedder(self._init_embed(input)) # embeddings是各个节点的嵌入(batch_size, graph_size, embed_dim)

        _log_p, pi = self._inner(input, embeddings)

        # print(f'input={input},pi={pi}') 此处mask没什么意义
    ...
    def _inner(self, input, embeddings):
        # input是最开始的输入,embeddings是各个节点的嵌入
        ...
        while not (self.shrink_size is None and state.all_finished()):
            # print(f'self.shrink_size={self.shrink_size}')
            ...
            #         # Filter states
            #         state = state.tihuan(unfinished)
            #         fixed = fixed.get_by_key(unfinished)
    ...
    def _get_log_p_topk(self, fixed, state, k=None, normalize=True):
        ...
        # Return topk 这个函数是用来求output中的最大值或最小值，返回两个参数：其一返回output中的最大值（或最小值），其二返回该值的索引。
    ...
    def _get_parallel_step_context(self, embeddings, state, from_depot=False):
        ...
        batch_size, num_steps = current_node.size() # num_steps是当前解码的步数吗?
